  ...
  return {
    ...
    init: function init() {
      ...
      var handleTitle = function handleTitle(tab, navigation, index) {
        ...
        var current = index + 1; // set wizard title

        $('.step-title', $('#form_wizard_1')).text('Step ' + (index + 1) + ' of ' + total); // set done steps
        ...
        if (current >= total) {
          ...
          $('#form_wizard_1').find('.button-submit').show(); //                    displayConfirm();
        } else {
      ...
      $('#form_wizard_1 .button-submit').click(function (e) {
        ...
        $.ajax({
          ...
          success: function success(data) {
            console.log(data);
            Print.postMessage(JSON.stringify(data).toString()); // if (data.meta.status_code === 200) {
            //     // mApp.unblockPage();
            //     //                            success.show();
            //     //                            error.hide();
            //     // window.location.href = 'welcome/success_projek';
            //     //                             window.history.go(-1);
            //     //                            location.reload();
            // } else {
            //     mApp.unblockPage();
